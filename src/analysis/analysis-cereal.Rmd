...
df_rep_decision[,3] <- ifelse(df_rep_lr$`p-value for GAP test`< 0.05 , 'fullmodel', 'linear or particularnonlinear')
...
In other words, we want to compare $\beta_{2i}$ if they are equal. We use F-Test for linearity assumption. We list the coefficient of final price, price before and discount from (1) and (2) and report the adjusted-Rsquared of each model and P-value for testing linear hypothesis on the coefficients of a system of equations by an F-test.
...
knitr::kable(df_linear_2, col.names = c('Brand','R2 of final price model','Rsquared of Pbefore and discount model', 'P-Value of testing linear hypothesis'))

remove(list=c("lm_est_step0_finalprice","lm_est_step0_pricebefore","coeftest"))
```


We got a problem when running with log-log specification as our discount can be zero and that make log0 becomes NaN. (Log0 is undefined. It's not a real number, because you can never get zero by raising anything to the power of anything else.) So, a linear model is used to avoid the aggregation bias that would occur when working with, for example, a log-log specification.
...
We try to explore more by running linear model from previous section or model (2) without interaction and with interaction between discount and price (model 3). We reported the adjusted R-squared of model (2) and (3) respectively.
...
We extend our model (4) by adding GAP measures including 1) `Gap with Historical benchmark price(before)` 2) `Gap with Competitive benchmark price(before)` 3)`Gap with Historical benchmark discount` and 4) `Gap with Competitive benchmark Discount`. We reported adjusted R-squared of this model (model (5)).
...
We then follow the criteria described by Pauwels et al. (2007) by using likelihood ratio test for linearity model (2), model(4) with cross products and model (5) with GAP measures. In case one of the LR tests is significant,we proceed with that particular nonlinear model. If both tests are significant, we proceed with the model with both HBP and CBP (with transition function)
...
df_decision[,3] <- ifelse(df_lr$`p-value for GAP test`< 0.05 , 'fullmodel', 'linear or particularnonlinear')
