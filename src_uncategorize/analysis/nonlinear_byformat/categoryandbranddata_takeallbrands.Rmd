...
```{r define function for filtering brand with complete (153) observations and generate relevant variable}

generate_df <- function(df,format,category,nweek){
#select the brands will be used to study
...
message(paste("# of remaining major brands used for study for",category,"in",format,":", dim(brandcompleteweek)[1],"(from",length(unique(df_format_category$brand)),")"))
...
totalmarketvalue <- sum(df_format_category$finalspending)
majorbrandmarketvalue <- sum(df_format_category_complete$finalspending)

message(paste("% of major brand market value:", (majorbrandmarketvalue/totalmarketvalue)*100))

#generate relevant variables
df <- df_format_category
...
df_competitor=df_bybrand[df_bybrand$brand != paste0(distinctbrandcomplete[i]),]
...
df_own=df_bybrand[df_bybrand$brand == paste0(distinctbrandcomplete[i]),]
df_own <- merge(df_own,df_competitorinfo, by=c("vecka"))
df_rep[(1+((i-1)*153)):(153+((i-1)*153)),] = df_own
df_rep[(1+((i-1)*153)):(153+((i-1)*153)),2] <- paste0(distinctbrandcomplete[i])
}
