...
```{r linear model loka}
lm_full_linear <- lm(dlogtotalvolume ~ dlogavgfinalprice + log(totalvolume1) + log(avgfinalprice1) ,data=df_full)
...
phi_1_full_lm <- summary(lm_full_linear)$coefficients[3,1]
phi_2_full_lm <- (summary(lm_full_linear)$coefficients[4,1])/(summary(lm_full_linear)$coefficients[3,1]) # tfull coef = phi1*phi2

paste("Initial Value for constant term:", c_full_lm) 
paste("Initial Value for base elasticity:", alpha_0_full_lm) 
paste("Initial Value for phi1:", phi_1_full_lm) 
paste("Initial Value for phi2:", phi_2_full_lm) #wrong sign but nonsig?
```
...
beta_l_his_full_lm <- as.numeric(summary(df_full$dlogavgfinalprice)[3])
beta_g_his_full_lm <- as.numeric(summary(df_full$dlogavgfinalprice)[1])
...
alpha_g_comp_full_lm <- -0.5
alpha_l_comp_full_lm <- 0.6

beta_l_comp_full_lm <- as.numeric(summary(df_full$cgap)[3])
beta_g_comp_full_lm <- as.numeric(summary(df_full$cgap)[1])
...
stage0_LS_nonlinear_full <- function(parvec){
  ...
  ut <- df_full$dlogtotalvolume - (parvec[1] + (parvec[2] 
                            ...
                            + parvec[7]*((log(df_full$totalvolume1))+parvec[8]*log(df_full$avgfinalprice1)))
  SSE <- sum(ut^2)
  return(SSE)
}
...
stage0LS_logL_nonlinear_full_constraint = optim(stage0_Start_v_LS,
...
stage0LS_logL_nonlinear_full_constraint$hessian
stage0_Start_v_LS
stage0_par_LS.est
...
LS_nonlinear_full <- function(parvec){
  ...
  ut <- df_full$dlogtotalvolume - (parvec[1] + (parvec[2] 
                            ...
                            + parvec[11]*((log(df_full$totalvolume1))+parvec[12]*log(df_full$avgfinalprice1)))
  SSE <- sum(ut^2)
  return(SSE)
}
...
```{r visualization hist and comp gap}

c_0 <- par_LS.est[1]
alpha0 <- par_LS.est[2]
alpha_HG <- par_LS.est[3]
beta_HG <- par_LS.est[4]
alpha_HL <- par_LS.est[5]
beta_HL <- par_LS.est[6]
alpha_CG <- par_LS.est[7]
beta_CG <- par_LS.est[8]
alpha_CL <- par_LS.est[9]
beta_CL <- par_LS.est[10]
phi_1_his <- par_LS.est[11]
phi_2_his <- par_LS.est[12]

# Defining x-values
graph_reso <- 0.01
x_values <- seq(min(df_full$dlogavgfinalprice), max(df_full$dlogavgfinalprice), by = graph_reso)
xgap_values <- seq(min(df_full$hgap), max(df_full$hgap), by = graph_reso)


# Defining various functions to plot
NLPE <- function(dlogavgfinalprice,hgap)
{( alpha0 + alpha_HG*( 1 + exp(gamma*(hgap-beta_HG) ) )^-1 + alpha_HL*( 1 + exp(-gamma*(hgap-beta_HL) ) )^-1 )*dlogavgfinalprice}

TransF <- function(hgap){alpha0 + alpha_HG*( 1 + exp(gamma*(hgap-beta_HG) ) )^-1 + alpha_HL*( 1 + exp(-gamma*(hgap-beta_HL)))^-1}
...
#Plot
plot(x_values, NLPE_values, type = "l", main = "Nonlinear price elasticities Loka", xlab = "dlogavgfinalprice", ylab = "NLPE")
plot(xgap_values, TransF_values, type = "l", main = "Smooth Transition Functions", xlab = "hispricegap", ylab = "STF")
...
stage0_LS_nonlinear_full_onlycomp <- function(parvec){
  
  F_C_G <- 1/(1+exp(gamma*(df_full$cgap - parvec[4])))
  F_C_L <- 1/(1+exp(-gamma*(df_full$cgap - parvec[6])))
  
  
  ut <- df_full$dlogtotalvolume - (parvec[1] + (parvec[2] 
                                         + parvec[3]*F_C_G + parvec[5]*F_C_L)*df_full$dlogavgfinalprice 
                            + parvec[7]*((log(df_full$totalvolume1))+parvec[8]*log(df_full$avgfinalprice1)))
  SSE <- sum(ut^2)
  return(SSE)
}
  ...
  parvec_2 <- alpha_0_full_lm
  parvec_3 <- -1
  parvec_4 <- beta_g_comp_full_lm 
  parvec_5 <- -0.5
  parvec_6 <-  beta_l_comp_full_lm 
  parvec_7 <- phi_1_full_lm
  parvec_8 <- phi_2_full_lm
...
stage0LS_logL_nonlinear_full_constraint_onlycomp = optim(stage0_Start_v_LS,
                  fn = stage0_LS_nonlinear_full_onlycomp, # function to maximize
                  method = "BFGS",
                  control = list(fnscale = 1), # minimize the function
                  hessian = T # calculate Hessian matrix because we will need for confidence intervals
                  )

MSE_calculated <- stage0LS_logL_nonlinear_full_constraint_onlycomp$value/(n-k)

paste("Calculated SE of model:",MSE_calculated)

stage0_par_LS.est_onlycomp<-stage0LS_logL_nonlinear_full_constraint_onlycomp$par
stage0LS_logL_nonlinear_full_constraint_onlycomp$hessian
OI<-solve(stage0LS_logL_nonlinear_full_constraint_onlycomp$hessian)

stage0_Start_v_LS
stage0_par_LS.est_onlycomp
...
LS_nonlinear_full <- function(parvec){
  ...
  parvec_5 <- -0.5
  parvec_6 <-  beta_l_his_full_lm 
  parvec_7 <- stage0_par_LS.est_onlycomp[3]
  parvec_8 <- stage0_par_LS.est_onlycomp[4] 
  parvec_9 <- stage0_par_LS.est_onlycomp[5]
  parvec_10 <- stage0_par_LS.est_onlycomp[6]
  parvec_11 <- phi_1_full_lm
  parvec_12 <- phi_2_full_lm
