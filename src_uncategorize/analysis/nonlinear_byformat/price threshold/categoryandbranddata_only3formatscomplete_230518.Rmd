...
```
#Function for merge category (TEMP AND NOT CLEAN)
```{r adding intensity, eval=FALSE, include=FALSE}
addcampaignintensity <- function(df_bybrand,category){
  df$nonpricecampaign <- ifelse(df$totaldiscount == 0 & df$week_campaign > 0, 1, 0)
  df$pricecampaign <- ifelse(df$totaldiscount != 0 & df$week_campaign > 0, 1, 0)
  weeklyweekcampaign <- setDT(df)[, .(totalweeknonpricecampaign=sum(nonpricecampaign),totalweekpricecampaign=sum(pricecampaign)), .(vecka,format,category)]
  weeklyweekcampaign_category <- weeklyweekcampaign[weeklyweekcampaign$category == category,]
  df_bybrand_intensity <- merge(df_bybrand,weeklyweekcampaign, by=c("vecka","format"))
return(df_bybrand_intensity)
}
```
