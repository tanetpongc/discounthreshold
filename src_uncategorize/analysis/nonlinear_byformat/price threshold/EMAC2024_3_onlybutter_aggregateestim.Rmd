...
df <- read.csv("../../../../data/df_scanner_butikid_complete_filteredcat_butter.csv")
...
df_bybrand$depth1 <- ave(df_bybrand$depth, df_bybrand$brand, FUN= lag_1)
...
df_bybrand$ddepth <- (df_bybrand$depth)-log(df_bybrand$depth1)
...
df_bybrand$dlogavgfinalprice_loss <- ifelse(df_bybrand$dlogavgfinalprice > 0, df_bybrand$dlogavgfinalprice, 0)
df_bybrand$dlogavgfinalprice_gain <- ifelse(df_bybrand$dlogavgfinalprice < 0, df_bybrand$dlogavgfinalprice, 0)
...
df_butter_conve <- genrelevantvar(df,format = "convenience",category ="Butter")
...
nonlinear_test_butter_conve <- test_nonlinearity_both(aggregate_df = df_butter_conve,discount_coef_result = discount_test_butter_conve, critical_p = 0.10)

#merge data
df_butter <- rbind(df_butter_hyper,df_butter_super,df_butter_conve)
discount_test <- rbind(discount_test_butter_hyper,discount_test_butter_super,discount_test_butter_conve)
nonlinear_test <- rbind(nonlinear_test_butter_hyper,nonlinear_test_butter_super,nonlinear_test_butter_conve)
```

```{r export the csv file, include=FALSE}
path = '../../../../gen/analysis'

output_discount = paste0(path,'/nonlinear_butter.csv')
output_nonlinear = paste0(path,'/disount_butter.csv')


write.csv(discount_test,output_discount)
write.csv(nonlinear_test,output_nonlinear)
```
...
#Model free
```{r plot and compare between change in final price and discount}
library(ggplot2)

ggplot(data = df_butter[df_butter$brand == "Svenskt smör",], aes(x = dlogavgfinalprice, y = dlogtotalvolume, color = format)) +
  geom_point() +
  geom_smooth(method = "lm", aes(group = format)) +
  theme_minimal() +
  labs(title = "Plot of Change in Final Price of Brand A on Sales by Format", x = "% Change of Price", y = "% Change of Quantitity Sold")

ggplot(data = df_butter[df_butter$brand == "Svenskt smör",], aes(x = dlogavgpricebefore, y = dlogtotalvolume, color = format)) +
  geom_point() +
  geom_smooth(method = "lm", aes(group = format)) +
  theme_minimal() +
  labs(title = "Plot of Change in Regular Price of Brand A on Sales by Format", x = "% Change of Regular Price", y = "% Change of Quantitity Sold")

ggplot(data = df_butter[df_butter$brand == "Svenskt smör",], aes(x = dlognondepth, y = dlogtotalvolume, color = format)) +
  geom_point() +
  geom_smooth(method = "lm", aes(group = format)) +
  theme_minimal() +
  labs(title = "Plot of Change in Discounts of Brand A on Sales by Format", x = "(Inverse) % Change of Discounts", y = "% Change of Quantitity Sold")

ggplot(data = df_butter, aes(x = dlogavgfinalprice, y = dlogtotalvolume, color = brand, shape = format)) +
  geom_point() +
  theme_minimal() +
...
distinctbrandcomplete<-unique(df_butter$brand)
par(mfrow = c(1,2))
for (i in 1:length(distinctbrandcomplete)){
plot(dtotalvolume ~ davgfinalprice, data=df_bybrand[df_bybrand$brand == distinctbrandcomplete[i],], main = distinctbrandcomplete[i], col="blue", xlab="Change in Final Price (SEK)", ylab="Change in Volume Sold")
  abline(lm(dtotalvolume ~ davgfinalprice, data=df_bybrand[df_bybrand$brand == distinctbrandcomplete[i],]), col="blue")
plot(dtotalvolume ~ -davgdiscount_inverse, data=df_bybrand[df_bybrand$brand == distinctbrandcomplete[i],], main = distinctbrandcomplete[i], col="red",xlab="()Change in Discount (SEK)", ylab="Change in Volume Sold")
  abline(lm(dtotalvolume ~ davgdiscount_inverse, data=df_bybrand[df_bybrand$brand == distinctbrandcomplete[i],]), col="red")
}
```

```{r}
#We chose Bregott for analysis
#We will use simplified model

bregott_hyper_mod2_sim <- lm(dlogtotalvolume ~ dlogavgfinalprice_gain + dlogavgfinalprice_loss + dlogavgcompfinalprice + log(totalvolume1) + log(avgfinalprice1) + cop_logavgfinalprice + holiday,data=df_butter_hyper[df_butter_hyper$brand == "Bregott",])

#stargazer(bregott_hyper_mod2_sim, single.row=TRUE)

dt_bregott_hyper <- data.table(x_gain = df_butter_hyper[df_butter_hyper$brand == "Bregott",]$dlogavgfinalprice_gain,x_loss = df_butter_hyper[df_butter_hyper$brand == "Bregott",]$dlogavgfinalprice_loss, x = df_butter_hyper[df_butter_hyper$brand == "Bregott",]$dlogavgfinalprice)
fit_bregott_hyper <- coef(bregott_hyper_mod2_sim)["dlogavgfinalprice_gain"]*dt_bregott_hyper$x_gain + coef(bregott_hyper_mod2_sim)["dlogavgfinalprice_loss"]*dt_bregott_hyper$x_loss
dt_bregott_hyper[,fit := fit_bregott_hyper]

# Plot with ggplot2 using pastel colors
ggplot(dt_bregott_hyper,aes(x = x, y = fit)) +
  geom_point() +  # Pastel color for points
  geom_line(color = "darkblue") +  # Pastel color for line
  theme_minimal() +
  labs(title = "Estimated Effect of Change in Final Price of Brand B in Hypermarket", x = expression(Delta * " ln(Final Price)"), y = expression(Delta * " ln(TotalVolumes)"))



bregott_super_mod4_sim <- lm(dlogtotalvolume ~ dlogavgpricebefore_gain + dlogavgpricebefore_loss + dlognondepth + dlogavgcompfinalprice + log(totalvolume1) + log(avgpricebefore1) + log(nondepth1) + cop_logavgpricebefore + cop_lognondepth + holiday,data=df_butter_super[df_butter_super$brand == "Bregott",])

#stargazer(bregott_super_mod4_sim, single.row=TRUE)

dt_bregott_super <- data.table(x_gain = df_butter_super[df_butter_super$brand == "Bregott",]$dlogavgpricebefore_gain,x_loss = df_butter_super[df_butter_super$brand == "Bregott",]$dlogavgpricebefore_loss, x_regprice = df_butter_super[df_butter_super$brand == "Bregott",]$dlogavgpricebefore, x_lognondepth = df_butter_super[df_butter_super$brand == "Bregott",]$dlognondepth, x_ddepth = df_butter_super[df_butter_super$brand == "Bregott",]$ddepth)


fit_bregott_super_regprice <- coef(bregott_super_mod4_sim)["dlogavgpricebefore_gain"]*dt_bregott_super$x_gain + coef(bregott_super_mod4_sim)["dlogavgpricebefore_loss"]*dt_bregott_super$x_loss
fit_bregott_super_nondepth <- coef(bregott_super_mod4_sim)["dlognondepth"]*dt_bregott_super$x_lognondepth
dt_bregott_super[,fit_regprice := fit_bregott_super_regprice]
dt_bregott_super[,fit_nondepth := fit_bregott_super_nondepth]

ggplot(dt_bregott_super,aes(x = x_regprice, y = fit_regprice)) +
  geom_point() +  # Pastel color for points
  geom_line(color = "darkblue") +  # Pastel color for line
  theme_minimal() +
  labs(title = "Estimated Effect of Change in Regular Price of Brand B in Supermarket", x = expression(Delta * " ln(Regular Price)"), y = expression(Delta * " ln(TotalVolumes)"))

ggplot(dt_bregott_super,aes(x = x_lognondepth, y = fit_bregott_super_nondepth)) +
  geom_point() +  # Pastel color for points
  geom_line(color = "darkblue") +  # Pastel color for line
  theme_minimal() +
  labs(title = "Estimated Effect of Change in Discounts of Brand B in Supermarket", x = expression(Delta * " ln(1-Depth)"), y = expression(Delta * " ln(TotalVolumes)"))


bregott_conve_mod1_sim <- lm(dlogtotalvolume ~ dlogavgfinalprice + dlogavgcompfinalprice + log(totalvolume1) + log(avgfinalprice1) + cop_logavgfinalprice + holiday,data=df_butter_conve[df_butter_conve$brand == "Bregott",])

#stargazer(bregott_conve_mod1_sim, single.row=TRUE)

dt_bregott_conve <- data.table(x = df_butter_conve[df_butter_conve$brand == "Bregott",]$dlogavgfinalprice)

fit_bregott_conve <- coef(bregott_conve_mod1_sim)["dlogavgfinalprice"]*dt_bregott_conve$x
dt_bregott_conve[,fit := fit_bregott_conve]


ggplot(dt_bregott_conve,aes(x = x, y = fit)) +
  geom_point() +  # Pastel color for points
  geom_line(color = "darkblue") +  # Pastel color for line
  theme_minimal() +
  labs(title = "Estimated Effect of Change in Final Price of Brand B in Convenience Store", x = expression(Delta * " ln(Final Price)"), y = expression(Delta * " ln(TotalVolumes)"))
```
