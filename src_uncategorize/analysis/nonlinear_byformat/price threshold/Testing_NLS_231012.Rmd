...
{c_0 
  + (( alpha0 + alpha_G*( 1 + exp(gamma*(hgap-beta_G) ) )^-1 + alpha_L*( 1 + exp(-gamma*(hgap-beta_L) ) )^-1 )*dlogavgfinalprice)
  + phi_1_his*(log(df_his$totalvolume1))+phi_2_his*log(df_his$avgfinalprice1)}
...
```{r test different starting value}

alpha_g_test <- 0.25
alpha_l_test <- 0.25

beta_l_test <- mean(df_his$dlogavgfinalprice) + sd(df_his$dlogavgfinalprice)
beta_g_test <- -beta_l_his


Start_v_LS_test <- c(parvec_1,parvec_2,alpha_g_test,beta_g_test,alpha_l_test,beta_l_test,parvec_7,parvec_8)

  n <- dim(df_his)[1]
  k <- length(Start_v_LS_test)


LS_logL_nonlinear_his_test = optim(Start_v_LS_test,
                  fn = LS_nonlinear_his, # function to maximize
                  method = "BFGS",
                  control = list(fnscale = 1), # minimize the function
                  hessian = T # calculate Hessian matrix because we will need for confidence intervals
...
MSE_calculated <- LS_logL_nonlinear_his_test$value/(n-k)

paste("Calculated SE of model:",MSE_calculated)

par_LS_test.est<-LS_logL_nonlinear_his_test$par


Start_v_LS_test
par_LS_test.est

alpha0 <- par_LS_test.est[2]
alpha_G <- par_LS_test.est[3]
beta_G <- par_LS_test.est[4]
alpha_L <- par_LS_test.est[5]
beta_L <- par_LS_test.est[6]
...
```
```{r check if the code is correct by running via linear model}
LS_linear_check <- function(parvec){
  ut <- df_his$dlogtotalvolume - (parvec[1] + parvec[2]*df_his$dlogavgfinalprice + parvec[3]*log(df_his$totalvolume1)+parvec[4]*log(df_his$avgfinalprice1))
  SSE <- sum(ut^2)
  return(SSE)
}
  parvec_1 <- 1
...
Start_v_check <- c(parvec_1,parvec_2,parvec_3,parvec_4)
LS_linear_check_est = optim(Start_v_check,
                  fn = LS_linear_check, # function to maximize
                  method = "BFGS",
                  control = list(fnscale = 1), # minimize the function
                  hessian = T # calculate Hessian matrix because we will need for confidence intervals
                  )
LS_linear_check_est$par #same
LS_linear_check_est$value/(152-4) #or 152

MLE_linear_check <- function(parvec){
  ut <- df_his$dlogtotalvolume - (parvec[1] + parvec[2]*df_his$dlogavgfinalprice + parvec[3]*log(df_his$totalvolume1)+parvec[4]*log(df_his$avgfinalprice1))
  logL <- sum (-0.5* log(parvec[5]) - 0.5*ut^2/parvec[5] )
  return(-logL)
}



  parvec_5 <- 1 #standard normal 

Start_v_MLEcheck <- c(parvec_1,parvec_2,parvec_3,parvec_4,parvec_5)
MLE_linear_check_est = optim(Start_v_MLEcheck,
                  fn = MLE_linear_check, # function to maximize
                  method = "BFGS",
                  control = list(fnscale = 1), # minimize the function
                  hessian = T # calculate Hessian matrix because we will need for confidence intervals
                  )
MLE_linear_check_est$par #same
```
