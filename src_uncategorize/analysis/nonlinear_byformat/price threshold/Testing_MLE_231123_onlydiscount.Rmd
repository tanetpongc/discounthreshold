...
LS_nonlinear_his <- function(parvec){
  ...
  ut <- df_his$dlogtotalvolume - (parvec[1] + (parvec[2] 
                            ...
                            + parvec[5]*((log(df_his$totalvolume1))+parvec[6]*log(df_his$avgpricebefore1)+parvec[7]*log(df_his$nondepth1)))
  SSE <- sum(ut^2)
  return(SSE)
}
  ...
  #parvec_6 <- phi_3_his_lm
  #parvec_7 <- 0.35
  

Start_v_LS <- c(parvec_1,parvec_2,parvec_3,parvec_4,parvec_5,parvec_6)
...
library(numDeriv)
hess <- hessian(x=par_LS.est, func=LS_nonlinear_his)
hess
...
LS_nonlinear_his <- function(parvec){
  ...
  logL <- sum (-0.5* log(parvec_7) - 0.5*ut^2/parvec_7)
  return(-logL)
}
...
NLPE_H <- function(dlogavgpricebefore)
      {( alpha0_H + alpha_H_L*( 1 + exp(-500*(dlogavgpricebefore) ) )^-1 )*dlogavgpricebefore}
TransF_H <- function(dlogavgpricebefore){alpha0_H + alpha_H_L*( 1 + exp(-500*(dlogavgpricebefore) ) )^-1}
...
NLPE_D <- function(dlognondepth)
      {( alpha0_D + alpha_D_L*( 1 + exp(-500*(dlognondepth) ) )^-1 )*dlognondepth}
TransF_D <- function(dlognondepth){alpha0_D + alpha_D_L*( 1 + exp(-500*(dlognondepth) ) )^-1}
...
plot(xgap_values_H, TransF_values_D, type = "l", main = "Smooth Transition Functions NONDEPTH", xlab = "dlognondepth", ylab = "STF")
