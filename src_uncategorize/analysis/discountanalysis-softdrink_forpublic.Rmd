---
title: "Discount Elasticity Analysis for Soft Drink"
author: "Tanetpong"
date: "23/11/2021"
output: pdf_document
...
categorydeal <- setDT(df_complete_discount)[,.(totaldiscvolume=sum(sales_volume)), .(vecka,format)]
categorytotal <- setDT(df_complete)[,.(totalvolume=sum(sales_volume)), .(vecka,format)]
categoryproportion<-left_join(categorytotal,categorydeal,by=c("format","vecka")) #merge will leave the week without deal
categoryproportion$totaldiscvolume[is.na(categoryproportion$totaldiscvolume)] <- 0
categoryproportionformat <- setDT(categoryproportion)[,.(averagedealprop=mean(totaldiscvolume/totalvolume)), .(format)]
...
categorycharacteristic_2 <-merge(categorycharacteristic_1,categoryproportionformat, by=c("format"))
categorycharacteristic <-merge(categorycharacteristic_2,categorycompstructure, by=c("format"))

remove(list=c("categorycharacteristic_1","categorycharacteristic_2","categorydeal","categorytotal","categoryproportion","categoryproportionformat", "categorydepth","categorybreadthweek","categorybreadthweekmerge","categorybreadth","categorycompstructure","df_complete_discount","weeklymarketbyweek"))
...
df_bybrand$davgdiscount_inverse <- -(df_bybrand$davgdiscount)
...
knitr::kable(categorycharacteristic, col.names = c('Format','Average Discount Depth','Average Discount Breath','Average Deal Dependence','Var of Market Share'))
```
